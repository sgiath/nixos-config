...
name: kimi-webbridge
description: |
  Kimi WebBridge lets AI control the user's real browser: navigate, click, type, read, screenshot, and interact with websites using the user's actual login sessions. Use this skill only when the user explicitly wants the local browser used, wants an already-open/logged-in browser session, or asks for browser screenshots/interaction that normal web search cannot provide.
---

# Kimi WebBridge

Control the user's real browser via the local daemon at `http://127.0.0.1:10086`.

This system installs `kimi-webbridge` declaratively through Home Manager/NixOS. Do not run the upstream `curl | bash` installer, `kimi-webbridge uninstall`, or `kimi-webbridge upgrade` unless the user explicitly asks to leave the declarative setup.
...
After getting the file path, use the local image viewing tool to inspect the image when needed.
...
Daemon and extension versions should normally be compatible. Read both via:
