...
}:
{
  config = lib.mkIf config.sgiath.agents.enable {
    ...
    programs.zsh.shellAliases = {
      oc = "OPENCODE_DISABLE_CLAUDE_CODE=true OPENCODE_ENABLE_EXA=true OPENCODE_EXPERIMENTAL=true OPENCODE_EXPERIMENTAL_FILEWATCHER=true OPENCODE_EXPERIMENTAL_LSP_TOOL=true OPENCODE_EXPERIMENTAL_EXA=true OPENCODE_EXPERIMENTAL_PLAN_MODE=true OPENCODE_EXPERIMENTAL_BACKGROUND_SUBAGENTS=true OPENCODE_EXPERIMENTAL_PARALLEL=true OPENCODE_EXPERIMENTAL_SCOUT=true OPENCODE_EXPERIMENTAL_WORKSPACES=true OPENCODE_PORT=24096 ${lib.getExe pkgs.llm-agents.opencode} --port 24096";
    };
  };
}
