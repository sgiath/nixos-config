...
- If your client supports MCP resources, read `backlog://workflow/overview` before creating or tracking tasks.
- If only tools are available, call `backlog.get_backlog_instructions()` and use `task-creation`, `task-execution`, or `task-finalization` selectors as needed.
- First time here: read the overview immediately.
- Already familiar: use cached overview `## Backlog.md Overview (MCP)`.
- Search before creating tasks to avoid duplicates.
